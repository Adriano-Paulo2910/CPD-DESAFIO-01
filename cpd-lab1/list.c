...
void update_terminated_process(list_t *list, int pid, time_t endtime)
{
    ...
    // Percorrer a lista para encontrar o processo com o pid correspondente
    while (item != NULL) {
        if (item->pid == pid) {
            // Encontrou o processo, atualiza o tempo de término
            item->endtime = endtime;
            printf("Processo com PID %d atualizado com sucesso.\n", pid);
            return;
        }
        item = item->next;
    }
    // Se o processo com o PID fornecido não for encontrado
